...
class CSGTEncoder(nn.Module):
    def __init__(self, in_dim, hidden_dim, num_layers=2, num_heads=4, dropout=0.5, use_layer_norm=True):
...
class MSVTNet(nn.Module):
    def __init__(self, num_channels=22, num_classes=4, input_time_length=1000, dropout_rate=0.5):
        ...
        self.embedding_dim = 32  # Start with smaller embedding dimension
    ...
    def compute_loss(self, outputs, aux_outputs, targets, weights=None):
        ...
        if self.training_step < 1000:
            alpha = 0.8  # Higher weight to auxiliary losses in early training
        else:
            alpha = 0.3  # Lower weight to auxiliary losses in later training
