#!/usr/bin/env python
# MSVTNet Training Summary Generator
...
def load_subject_data(results_dir, subject_id, mode="session_dependent"):
...
def extract_metrics_from_report(report_text):
    ...
    # Process class metrics (looking for 4 classes)
        ...
        for line in lines:
            if line.strip().startswith(name):
                ...
                try:
                    ...
                    print(f"Warning: Could not parse metrics for {name}")
...
def generate_class_performance_table(subjects_data):
    ...
    headers = ["Subject", "Left Hand", "Right Hand", "Feet", "Tongue", "Average"]
    ...
    if rows:
        avg_row = ["Average"]
        for col in range(1, 6):  # 4 classes + average accuracy
...
def plot_accuracy_comparison(subjects_data, mode, output_dir):
    ...
    plt.axhline(y=0.25, color='green', linestyle='--', label='Random Chance (0.25)')
...
def plot_class_performance(subjects_data, mode, output_dir):
    ...
    for subject_id, data in sorted(subjects_data.items()):
        ...
        class_data["Feet"].append(metrics.get("class_2_recall", 0))
        class_data["Tongue"].append(metrics.get("class_3_recall", 0))
    ...
    plt.bar(x - 1.5*bar_width, class_data["Left Hand"], bar_width, label='Left Hand', color='blue', alpha=0.7)
    plt.bar(x - 0.5*bar_width, class_data["Right Hand"], bar_width, label='Right Hand', color='green', alpha=0.7)
    plt.bar(x + 0.5*bar_width, class_data["Feet"], bar_width, label='Feet', color='red', alpha=0.7)
    plt.bar(x + 1.5*bar_width, class_data["Tongue"], bar_width, label='Tongue', color='purple', alpha=0.7)
...
def generate_summary_report(results_dir, output_dir, modes=None):
    ...
    if modes is None:
        modes = ["session_dependent", "session_independent"]
    
    print(f"\nGenerating MSVTNet training summary for {', '.join(modes)} modes...\n")
    ...
    for mode in modes:
        ...
        with open(report_file, 'w') as f:
            f.write(f"MSVTNet Training Results Summary - {mode}\n")
            ...
            if count > 0:
                ...
                f.write(f"- Performance relative to chance (0.25): {(avg_acc - 0.25) / 0.25 * 100:.2f}% improvement\n\n")
    ...
    if "session_dependent" in all_subjects_data and "session_independent" in all_subjects_data:
        print("\nGenerating combined comparison between approaches...")
        plot_combined_accuracy_comparison(
            all_subjects_data["session_dependent"],
            all_subjects_data["session_independent"],
            output_dir
        )
        ...
        with open(combined_report_file, 'w') as f:
            f.write("MSVTNet Combined Analysis: Session-Dependent vs Session-Independent\n")
            f.write(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"User: {os.environ.get('USERNAME', 'Chandresh202004')}\n\n")
            ...
            for subject_id in range(1, 10):
                dep_data = all_subjects_data["session_dependent"].get(subject_id, {})
                indep_data = all_subjects_data["session_independent"].get(subject_id, {})
            ...
            for subject_id in range(1, 10):
                ...
                if subject_id in all_subjects_data["session_dependent"] and "history" in all_subjects_data["session_dependent"][subject_id]:
                    dep_acc = all_subjects_data["session_dependent"][subject_id]["history"].get("best_acc", 0)
                
                if subject_id in all_subjects_data["session_independent"] and "history" in all_subjects_data["session_independent"][subject_id]:
                    indep_acc = all_subjects_data["session_independent"][subject_id]["history"].get("best_acc", 0)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate MSVTNet training summary report")
    parser.add_argument("--results_dir", type=str, default="D:/MSVTNet_Project/results/msvtnet",
                        help="Directory containing results for different approaches")
    parser.add_argument("--output_dir", type=str, default="D:/MSVTNet_Project/summary",
                        help="Directory to save summary report")
    parser.add_argument("--mode", type=str, choices=["session_dependent", "session_independent", "both"],
                        default="both", help="Which training mode to analyze")
    
    args = parser.parse_args()
    
    modes = ["session_dependent", "session_independent"] if args.mode == "both" else [args.mode]
